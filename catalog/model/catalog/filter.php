...
class ModelCatalogFilter extends Model {
	...
	public function getNameItemNames($categories_id) {
		
		$categories_hash = md5($categories_id);
		
		$cache_data = $this->cache->get('get_name_items_names.' . $categories_hash . '.' . $this->config->get('config_language_id'));
		
		if (!$cache_data && !is_array($cache_data)) {
		
			$query = $this->db->query("SELECT DISTINCT pd.name FROM `" . DB_PREFIX . "product_description` pd WHERE pd.product_id IN (SELECT product_id FROM `" . DB_PREFIX . "product_to_category` WHERE category_id IN (" . $this->db->escape($categories_id) . ")) ORDER BY pd.name ASC");
			
			$manufacters = $query->rows;
			
			$this->cache->set('get_name_items_names.' . $categories_hash . '.' . $this->config->get('config_language_id'), $manufacters);
		}
		else {
	...
	public function getQuantityItemNames($categories_id) {
		
		$categories_hash = md5($categories_id);
		
		$cache_data = $this->cache->get('get_quantity_items_names.' . $categories_hash . '.' . $this->config->get('config_language_id'));
		
		if (!$cache_data && !is_array($cache_data)) {
		
			$query = $this->db->query("SELECT MAX(pd.quantity) as max_quantity,MIN(pd.quantity) as min_quantity FROM `" . DB_PREFIX . "product` pd WHERE pd.product_id IN (SELECT product_id FROM `" . DB_PREFIX . "product_to_category` WHERE category_id IN (" . $this->db->escape($categories_id) . "))");			
			
			$quantity_value = array(
				0 => array('name' => 'min_price',
						   'value'=> $query->rows[0]['min_quantity'],
						   'image'=> ''),
				1 => array('name' => 'max_price',
						   'value'=> $query->rows[0]['max_quantity'],
						   'image'=> '')
			);
			
			$this->cache->set('get_quantity_items_names.' . $categories_hash . '.' . $this->config->get('config_language_id'), $quantity_value);
		}
		else {
			$quantity_value = $cache_data;
		}
