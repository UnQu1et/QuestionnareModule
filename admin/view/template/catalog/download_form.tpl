	...
	<div class="content">
	  <form action="<?php echo $action; ?>" method="post" enctype="multipart/form-data" id="form">
		<table class="form">
		  ...
		  <tr>
			<td>Выберите статью из списка</td>
			<td>
				<select name="information_id">
					<option>--Выберите статью--</option>
					<? foreach($informations as $v){?>					
					<option <? if($information_id == $v['information_id']){?>selected="selected" <?}?> value="<?php echo $v['information_id']; ?>"><?php echo $v['title']; ?></option>
					<?}?>
				</select>
			</td>
		  </tr>
